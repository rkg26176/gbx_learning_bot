...
# ==========================================
# 📋 PAGINATED USERLIST COMMAND (/userlist)
# ==========================================
def send_userlist_page(chat_id, page=0, message_id=None):
  ...
  # Normal Users Pagination Logic
  start_idx = page * ITEMS_PER_PAGE
  end_idx = start_idx + ITEMS_PER_PAGE
  current_normal = normal_users[start_idx:end_idx]
  
  text = f"📋 **All Registered Users List (Page {page + 1})**\n\n"
  
  text += f"🟢 **1. Normal / Active Users ({len(normal_users)} total):**\n"
  if current_normal:
    for idx, uid in enumerate(current_normal, start_idx + 1):
      text += f"{idx}. User ID: `{uid}`\n"
  else:
    text += "Is page par koi normal user nahi hai.\n"
  ...
  nav_buttons = []
  if page > 0:
    nav_buttons.append(InlineKeyboardButton(text="⬅️ Prev", callback_data=f"userlist_page:{page - 1}"))
  if end_idx < len(normal_users):
    nav_buttons.append(InlineKeyboardButton(text="Next ➡️", callback_data=f"userlist_page:{page + 1}"))
...
def userlist_command(message):
  if message.chat.id != ADMIN_CHAT_ID:
    return
  send_userlist_page(message.chat.id, page=0)


@bot.callback_query_handler(func=lambda call: call.data.startswith("userlist_page:"))
def handle_userlist_pagination(call):
  ...
  try:
    ...
    pass
  send_userlist_page(call.message.chat.id, page=page, message_id=call.message.message_id)
...
def handle_admin_menu_callbacks(call):
  if call.from_user.id != ADMIN_CHAT_ID:
    return
  send_userlist_page(call.message.chat.id, page=0)
